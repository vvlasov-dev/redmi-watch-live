...
| `index.dc.html` | фронт (design-canvas HTML, `{{ }}`-биндинги) |
...
- **`test_sleep_engine.py` + `test_activity.py`** — golden-датасет реальных ночей и
  байт-точные фикстуры как регресс-ограждение; `deploy.ps1` не деплоит на красных
  тестах, `hooks/pre-commit` не даёт закоммитить.
